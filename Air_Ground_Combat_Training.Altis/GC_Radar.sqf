...
_fnc_getTargets = {
...

	{
		...
		if (((_x distance _AWACST) <= _radarsize) && (alive _x) && (_unitHeight <= _radarFloor)) then
		{
			_yea = false;
...
[_getTargets] spawn {
	...
	_VRDtext0 = [parseText format["<t align = 'center' color = '#00CC00'>GROUND CONTROL RADAR SYSTEM INITIATED</t>"], [0.25, 1, 0.5, 0.05], [1, 1], 2] spawn 	BIS_fnc_textTiles;
	sleep 1;
